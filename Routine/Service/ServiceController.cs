...
namespace Routine.Service
{
	public class ServiceController : Controller
	{
		...
		public static string HandleAction { get { return "Handle"; } }
		public static string ApplicationModelAction { get { return "ApplicationModel"; } }
		public static string ConfigurationAction { get { return "Configuration"; } }
		public static string FileAction { get { return "File"; } }
		...
		{
			return File("app/application/index.html");
		}

		[OutputCache(Location = OutputCacheLocation.ServerAndClient, VaryByParam = "*", Duration = 60 * 60)]
		public ActionResult File(string path)
		{
			...
			return File(Encoding.UTF8.GetBytes(fileContent), GetMimeType(path.AfterLast(".")));
		}

		[OutputCache(Location = OutputCacheLocation.ServerAndClient, VaryByParam = "*", Duration = 60 * 60)]
		public ActionResult Fonts(string fileName)
		{
			return File(GetStream("assets/fonts/" + fileName), GetMimeType(fileName));
		}
		...
		public ActionResult Configuration()
		{
			...
			{
				url = UrlBase,
				requestHeaders = context.ServiceConfiguration.GetRequestHeaders(),
				responseHeaders = context.ServiceConfiguration.GetResponseHeaders()
			}, JsonRequestBehavior.AllowGet);
		}

		public ActionResult ApplicationModel()
		{
			IndexApplicationModelIfNecessary();

			return LargeJson(context.ObjectService.ApplicationModel, JsonRequestBehavior.AllowGet);
		}

		public ActionResult Handle(string modelId, string idOrViewModelIdOrOperation, string viewModelIdOrOperation, string operation)
		{
			IndexApplicationModelIfNecessary();
			...
			ActionResult result;
			var appModel = context.ObjectService.ApplicationModel;
			...
			{
				return AmbiguousModel(modelId, ex.AvailableModels);
			}
			catch (ModelNotFoundException ex)
			{
				return TargetModelNotFound(ex.ModelId);
			}
			...
			{
				var allowGet = context.ServiceConfiguration.GetAllowGet(resolution.ViewModel, resolution.OperationModel);

				if (!IsPost && !IsGet) { return MethodNotAllowed(allowGet); }
				if (IsGet && !allowGet) { return MethodNotAllowed(false); }
				...
				var variableData = context.ObjectService.Do(resolution.Reference, resolution.OperationModel.Name, parameterValues);
				var compressor = new DataCompressor(appModel, resolution.OperationModel.Result.ViewModelId);

				result = LargeJson(compressor.Compress(variableData), allowGet ? JsonRequestBehavior.AllowGet : JsonRequestBehavior.DenyGet);
			}
			else
			{
				if (!IsPost && !IsGet) { return MethodNotAllowed(true); }

				ProcessRequestHeaders();

				var objectData = context.ObjectService.Get(resolution.Reference);
				var compressor = new DataCompressor(appModel, resolution.Reference.ViewModelId);

				result = LargeJson(compressor.Compress(objectData), JsonRequestBehavior.AllowGet);
			}
		...
		private IDictionary<string, object> GetParameterDictionary()
		{
			if (IsGet)
			{
				return Request.QueryString.AllKeys.ToDictionary(s => s, s => Request.QueryString[s] as object);
			}
			...
			return (IDictionary<string, object>)serializer.DeserializeObject(requestBody) ?? new Dictionary<string, object>();
		}

		private bool IsGet { get { return "GET".Equals(Request.HttpMethod, StringComparison.InvariantCultureIgnoreCase); } }
		private bool IsPost { get { return "POST".Equals(Request.HttpMethod, StringComparison.InvariantCultureIgnoreCase); } }

		private string UrlBase
		{
			get { return Url.RouteUrl(Constants.SERVICE_ROUTE_NAME_BASE + "handle", new { modelId = "REMOVE" }).BeforeLast("/REMOVE"); }
		}

		private void ProcessRequestHeaders()
		{
			var requestHeaders = Request.Headers.AllKeys.ToDictionary(key => key, key => HttpUtility.HtmlDecode(Request.Headers[key]));

			foreach (var processor in context.ServiceConfiguration.GetRequestHeaderProcessors())
			{
		...
		private void AddResponseHeaders()
		{
			foreach (var responseHeader in context.ServiceConfiguration.GetResponseHeaders())
			{
				...
				{
					Response.Headers.Add(responseHeader, HttpUtility.UrlEncode(responseHeaderValue));
				}
			}
		}
		...
		protected sealed class LargeJsonResult : JsonResult
		{
			...
			public LargeJsonResult(IJsonSerializer serializer)
			{
				this.serializer = serializer;
			}
