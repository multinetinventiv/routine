namespace Routine
{
	internal class Constants
	{
        public const string DEFAULT_VIRTUAL_MARK = "__routine_virtual";
		public const int DEFAULT_MAX_FETCH_DEPTH = 10;
        public const int DEFAULT_MAX_RESULT_LENGTH = 2 * 1024 * 1024;

		public const int FIRST_DEPTH = 1;
		
        public const string CACHE_DOMAIN_TYPES = "__routine_domain_types";
        public const string CACHE_APPLICATION_MODEL = "__routine_application_model";

		public const string INITIALIZER_ID = "__routine_initializer";

		public const string MODEL_NAME_VOID = "__routine_void";
        public const string MODEL_NAME_NULL = "__routine_null";

        public const string MODEL_ID_VOID = MODEL_NAME_VOID;
        public const string MODEL_ID_NULL = MODEL_NAME_NULL;

		public const string PROPERTY_AS_METHOD_DEFAULT_PREFIX = "Get";
	}
}
